...
The original data in wgrib2 format needs to be decorded and converted to [LETKF observation file format](Observation-file-format.md) before the experiment. Superobservation, observation selection or thinning treatment may be performed along with it.  
...
To initiate a data assimilation cycle, the ensemble of initial model states, in a form of restart files, is necessary. The natural way to create initial conditions to start with is adding random perturbation to the true state. In this testcase, a pre-made set of restart files in `$DATADIR/scale-letkf-test-suite/exp/18km_Japan/init` can be used. 

### Boundary condition

The model needs the data of 3-D atmospheric variables for lateral boundary conditions. In this experiment, NCEP GFS forecast data is used as a source. Boundary data files in SCALE-RM format are made by `scale-rm_init_ens`. There are pre-made boundary files in `$DATADIR/scale-letkf-test-suite/exp/18km_Japan/bdy/mean`. 
...
The periods of the cycle and observation window are set in this part of `config.main`. This experiment utilizes "4D-LETKF", in which observations within a time window with a finite period will be assimilated. With the settings below, observation less than 3 hours away from the assimilation time is included. When the cycle starts at 00Z for example, observations from 03Z to 09Z are assimilated to create analysis at 06Z. For this perpose, the ensemble forecast is performed up to 9 hours from 00Z, as the LETKF needs ensemble first-guess field between 03-09Z. 
...
While it is running, the progress status can be monitored with the log files of letkf in each cycle created in `$OUTDIR/<atime>/log/letkf/` (note that atime is `$STIME` plus 6 hours). 
...
Multiple forecasts can be performed simultaneously from a set of different initial times from `STIME` to `ETIME` every `LCYCLE`. If `STIME` to `ETIME` are same, only an ensemble forecast from a single initial time is performed.  
...
After the forecast job completes, the resultand files can be found in `$OUTDIR/<time>/fcst`, where <time> is the initial time of the forecast. Note that it is separated from `$OUTDIR/<time>/hist` which is used in data assimilation. In the default setting, both history and restart files are made in fcst directory. The output setting is controlled by `OUT_OPT` in `config.fcst`. 
