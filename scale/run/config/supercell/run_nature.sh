...
cp ${SCALEDIR}/scale-rm/test/data/input_sounding/supercell.txt $TMP/
...
mkdir -p $TMP/log
mkdir -p $OUTDIR/nature/init
mkdir -p $OUTDIR/nature/hist
mkdir -p $OUTDIR/topo

ln -s $SCALEDIR/bin/scale-rm_init $TMP/. 
ln -s $SCALEDIR/bin/scale-rm $TMP/. 

cp $SCRP_DIR/config.nml.scale_init $TMP/init_nature.conf
cp $SCRP_DIR/config.nml.scale      $TMP/run_nature.conf

cat $SCRP_DIR/config.nml.scale_init | sed \
    -e "/!--RESTART_OUT_BASENAME--/a RESTART_OUT_BASENAME=\"$OUTDIR/nature/init/init\","  \
    -e "/!--TOPOGRAPHY_OUT_BASENAME--/a TOPOGRAPHY_OUT_BASENAME=\"$OUTDIR/topo/topo\"," \
> $TMP/init_nature.conf
...
mpirunf - ./scale-rm_init init_nature.conf log/init_nature || exit \$?
mpirunf - ./scale-rm       run_nature.conf log/run_nature  || exit \$?
