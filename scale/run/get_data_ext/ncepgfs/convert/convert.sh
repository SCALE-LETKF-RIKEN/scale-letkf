...
##### modification needed for updated GFS format #####
  LEVS_IGNORE="0.01 mb|0.02 mb|0.04 mb|0.07 mb|0.1 mb|0.2 mb|0.4 mb|0.7 mb|15 mb|40 mb"
#####
...
#  wgrib2 ${gfile} -match ":MSLET:"  -match ":mean sea level:"            -grib ${tfile}
  ...
  wgrib2 ${gfile} -match ":UGRD:" -match "mb" -not "mb above ground" | grep -vE "${LEVS_IGNORE}" | sort -n -r | wgrib2 -i ${gfile} -append -grib ${tfile}
  wgrib2 ${gfile} -match ":VGRD:" -match "mb" -not "mb above ground" | grep -vE "${LEVS_IGNORE}" | sort -n -r | wgrib2 -i ${gfile} -append -grib ${tfile}
  wgrib2 ${gfile} -match ":TMP:"  -match "mb" -not "mb above ground" | grep -vE "${LEVS_IGNORE}" | sort -n -r | wgrib2 -i ${gfile} -append -grib ${tfile}
  wgrib2 ${gfile} -match ":RH:"   -match "mb" -not "mb above ground" | grep -vE "${LEVS_IGNORE}" | sort -n -r | wgrib2 -i ${gfile} -append -grib ${tfile}
