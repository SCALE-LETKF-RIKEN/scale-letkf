...
for item in topo landuse ; do
  ...
  for pe in $(seq -f %06g 0 7) ; do
    ln -s $DATADIR/exp/18km_Japan/init/const/$item/${item}.pe${pe}.nc ./const/$item/
  done
done
...
for it in $(seq 0 2); do
  ...
  ln -s $DATADIR/exp/18km_Japan/bdy/${time}/mean/atm_${atime}.grd  mean/bdyorg_atm_${time:0:8}-${time:8:6}.000_0000${it}.grd
  ln -s $DATADIR/exp/18km_Japan/bdy/${time}/mean/land_${atime}.grd mean/bdyorg_lnd_${time:0:8}-${time:8:6}.000_0000${it}.grd
  ln -s $DATADIR/exp/18km_Japan/bdy/${time}/mean/sfc_${atime}.grd  mean/bdyorg_sfc_${time:0:8}-${time:8:6}.000_0000${it}.grd
done
...
for mem in mean $(seq -f %04g 1 5) ;do
  ...
  mkdir -p $mem/hist
  for pe in $(seq -f %06g 0 7) ; do
    ln -s $DATADIR/exp/18km_Japan/init/$time/anal/${mem}/init_${time:0:8}-${time:8:6}.000.pe${pe}.nc ./$mem/anal/
  done
done
