...
!
    ...
    call get_obsdep_efso_mpi( nobstotal, nobs0, obsset, obsidx, obsdep, obshdxf )
  endif
  ...
  if ( nobstotal > 0 ) then
    ...
    obsda%idx(:) = obsidx
    obsda%val(:) = obsdep
    obsda%qc(:)  = iqc_good !obsqc ! qc should always be good because QC has been applied in a previous cycle
